...
<style>
	...
	if (isset($_COOKIE['login'])) {
			...
			if (md5($email . $salt) == $hashedEmail) {			
				...
				$dump .= '<a href="phpoefening036-artikels-toevoegen-form.php">Voeg een artikel toe</a><br />
						<ul>';
				...
				while ($row = mysql_fetch_assoc($articleQuery)) {
					...
					$dump .= '<li>
								<a href="phpoefening036-artikels-wijzigen.php?artikel=' . $row['id'] . '">artikel wijzigen</a> | 
								<a href="phpoefening036-artikels-activeren.php?artikel=' . $row['id'] . '">artikel ' . (($row['is_active'] == 0)?'':'de') . 'activeren</a> | 
								<a href="phpoefening036-artikels-verwijderen.php?artikel=' . $row['id'] . '">artikel verwijderen</a>
							</li>';
